...
import { MessageCircle, Send, User, Search, Loader2, RefreshCw } from "lucide-react";
...
export function AdminChatClient({ 
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    // Optimistic UI — gunakan ID admin yang sebenarnya
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-1">
                      <h4 className="font-semibold text-sm truncate pr-2 text-foreground flex items-center gap-2">
                        {conv.user1.name}
                        {conv.user1.role === "Penjual" && (
                          <span className="bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0.5 rounded font-bold">Toko</span>
                        )}
                      </h4>
                      {latestMsg && (
                        <span className="text-[10px] text-muted-foreground shrink-0">
                          {formatDistanceToNow(new Date(latestMsg.createdAt), { addSuffix: true, locale: id })}
                        </span>
                      )}
                    </div>
                    {latestMsg && (
                      <p className={`text-xs truncate ${latestMsg.senderId === adminUserId ? 'text-emerald-600' : 'text-muted-foreground'}`}>
                        {latestMsg.senderId === adminUserId ? 'Anda: ' : ''}{latestMsg.content}
                      </p>
                    )}
                  </div>
                </button>
            ...
            <div className="p-4 border-b flex items-center gap-3 bg-white">
              ...
              <div>
                <h3 className="font-bold text-foreground flex items-center gap-2">
                  {activeConversation.user1.name}
                  {activeConversation.user1.role === "Penjual" && (
                    <span className="bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0.5 rounded font-bold">Toko</span>
                  )}
                </h3>
                <p className="text-xs text-muted-foreground">{activeConversation.user1.email}</p>
              </div>
            </div>
